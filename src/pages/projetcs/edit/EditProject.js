...
const EditProject = () => {
  ...
  const handleChange = (e) => {
    setAlluser({ ...alluser, [e.target.name]: e.target.value });
    setId(user?.member_id);
  };

  const fetchData = useCallback(async () => {
    const res = await get_project_by_id(data);
  ...
  const handleSubmit = (e) => {
    if (
      alluser.project_name &&
      alluser.registration_fee &&
      alluser.total_share
    ) {
      update_project(alluser, id).then((res) => {
            ...
            >
              ...
              >
                <TextField
                  ...
                  onChange={handleChange}
                  value={alluser?.registration_fee}
